...
$(function (){
    ...
    $.when(ajaxGetSection).done(function(){
        if(sectionInfo.videoid){
            getplaytoken(sectionInfo.videoid)
        }else{
            layer.msg('视频地址不存在或已经删除');
        }
    })
})
function play(vid,token){
    var player = new Aliplayer({
        ...
        player.on('ready',function() {
            $.ajax({
                ...
                type:"post",
                data:{'sid':sectionInfo.id,'cid':sectionInfo.csid,'type':1},
                dataType:'json',
        ...
        player.on('timeupdate',function(){
            ...
            var duration=sectionInfo.duration>0?sectionInfo.duration:300;
            if(seek>seeked  && seek % duration==0) {
                ...
                $.ajax({
                    ...
                    type: "post",
                    data: {'sid': sectionInfo.id, 'cid': sectionInfo.csid, 'type': 1, 'duration': duration,'seek':seek},
                    dataType: 'json',
                    success: function (data) {
                        if(data.code==0){
    ...
    ajaxGetSection = $.ajax({
        ...
        success: function (result) {
            if (result.code === 0) {
                ...
                progress=(learnInfo.status==1 || sectionInfo.isforward==1)?'progress':'';
